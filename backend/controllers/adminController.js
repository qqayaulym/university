...
export const getAllUsers = async (req, res) => {
  ...
  try {
    const result = await sql`SELECT id, username, email, role FROM users`;
    res.json(result);
  } catch (err) {
    console.error(err);
...
export const updateUserRole = async (req, res) => {
  ...
  try {
    const targetUserResult = await sql`SELECT id, role FROM users WHERE id=${id}`;
    ...
    if (targetUser.role === role) {
      const sameRoleUser = await sql`SELECT id, username, email, role FROM users WHERE id=${id}`;
      return res.json(sameRoleUser[0]);
    }

    const result = await sql`UPDATE users SET role=${role} WHERE id=${id} RETURNING id, username, email, role`;
